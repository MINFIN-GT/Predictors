...
private static Options options;
	...
	static{
		options = new Options();
		options.addOption(Option.builder("ing_ra").hasArgs().longOpt("ing_ra").desc("calcula los pronosticos para los ingresos por recurso - auxiliar").build());
		options.addOption(Option.builder("ing_r").hasArgs().longOpt("ing_r").desc("calcula los pronosticos para los ingresos por recurso").build());
		options.addOption(Option.builder("ing").hasArgs().longOpt("ing").desc("calcula los pronosticos para los ingresos generales").build());
		...
		try{
			...
			if (CMemSQL.connect()){
				...
				if(cline.hasOption("ing_ra")){
					...
					CIngreso.getPronosticosRecursoAuxiliar(conn, recurso, auxiliar, año, mes, numero_datos, true);
				}
				    ...
				    Integer numero_datos=(argumentos.length>2) ? Integer.parseInt(argumentos[2]) : 12;
				    Integer recurso=(argumentos.length>3) ? Integer.parseInt(argumentos[3]) : 0;
					CIngreso.getPronosticosRecurso(conn, recurso, año, mes, numero_datos, true);
				}
					...
					Integer mes=(argumentos.length>1) ? Integer.parseInt(argumentos[1]) : start.getMonthOfYear()+1;
				    Integer numero_datos=(argumentos.length>2) ? Integer.parseInt(argumentos[2]) : 12;
					CIngreso.getPronosticosRecursosTotales(conn, año, mes, numero_datos, true);
				}
